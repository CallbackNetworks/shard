    ...
    <div className="kt-page">
      ...
      {newKey && (
        <div className="kt-modal-backdrop">
          <div className="kt-modal">
            ...
            <div className="kt-panel" style={{ display: 'flex', gap: 6, alignItems: 'flex-start', marginBottom: 12, padding: 10, background: 'rgba(251,191,36,0.08)', borderColor: 'rgba(251,191,36,0.2)' }}>
            ...
            <button onClick={() => setNewKey(null)} className="kt-btn kt-btn-primary" style={{ width: '100%' }}>
        ...
        <div className="kt-panel" style={{ padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button onClick={handleCreate} disabled={!form.name || form.scopes.length === 0}
              className="kt-btn kt-btn-primary" style={{ opacity: (!form.name || form.scopes.length === 0) ? 0.4 : 1 }}>{t('create')}</button>
            <button onClick={() => setShowCreate(false)} className="kt-btn">{t('cancel')}</button>
          </div>
        </div>
      )}
      ...
      {apiKeys.length === 0 && !showCreate ? (
        <div className="kt-empty">
          <Key size={36} className="kt-empty-icon" />
          <p className="kt-empty-title">{t('apiKeys.empty')}</p>
          <p style={{ marginTop: 6, fontSize: 13 }}>{t('apiKeys.emptyHint')}</p>
          <button
            onClick={() => setShowCreate(true)}
            className="kt-btn kt-btn-primary" style={{ marginTop: 16 }}
          >
            {t('apiKeys.new')}
          </button>
        </div>
