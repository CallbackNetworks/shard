        ...
        <tr style={{ background: 'rgba(255,255,255,0.02)' }}>
          <td colSpan={7} style={{ padding: '8px 16px 16px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase' }}>Request URL</div>
                <div style={{ fontSize: 11, color: '#9ca3af', wordBreak: 'break-all' }}>{delivery.request_url}</div>
              </div>
              {delivery.error && (
                <div>
                  <div style={{ fontSize: 10, color: '#f87171', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase' }}>Error</div>
                  <div style={{ fontSize: 11, color: '#f87171' }}>{delivery.error}</div>
                </div>
              )}
              {delivery.response_body && (
                <div style={{ gridColumn: '1 / -1' }}>
                  <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase' }}>Response Body</div>
                  <pre style={{
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 4, fontWeight: 600, textTransform: 'uppercase' }}>Payload</div>
                <pre style={{
                  fontSize: 11, color: '#9ca3af', background: 'rgba(0,0,0,0.3)', borderRadius: 6,
  ...
  return (
    <div style={{ padding: 28, maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Webhook Delivery Logs</h1>
          <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>
            Outbound webhook delivery history and retry management.
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            onClick={() => { if (window.confirm('Purge deliveries older than 30 days?')) purge.mutate() }}
            style={{ ...btn('danger'), display: 'flex', alignItems: 'center', gap: 4 }}
          >
            <Trash2 size={12} /> Purge Old
          </button>
        </div>
      </div>
      ...
      <div style={{ background: '#0f1117', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, overflow: 'hidden' }}>
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: 64, color: '#4b5563' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 500 }}>No deliveries found</div>
          </div>
        ) : (
