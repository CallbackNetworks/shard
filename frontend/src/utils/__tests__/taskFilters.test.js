...
describe('filterTasks', () => {
  it('returns the list unchanged when every dimension is "all"', () => {
    ...
    expect(ids(filterTasks(tasks, { status: 'all', priority: 'all' }))).toEqual(['1', '2', '3'])
  })

  it('filters by status, priority, assignee, and agent', () => {
    expect(ids(filterTasks(tasks, { status: 'todo' }))).toEqual(['1'])
  ...
  describe('due keyword', () => {
    ...
    it('overdue keeps only past-due dated tasks', () => {
      expect(ids(filterTasks(tasks, { due: 'overdue' }))).toEqual(['2'])
    })
