        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {files.map(f => (
            <div key={f.id} style={{
              ...
              <button onClick={() => handleDelete(f.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'color-mix(in srgb, var(--kt-hit) 50%, transparent)', padding: 0, display: 'flex' }}>
                <X size={10} />
              </button>
            </div>
          ))}
