...
- **Phase A — 地基(先交付用戶自訂層):** `node_types` 註冊表(type 資料化、seed 內建)+ 通用 Node CRUD API + **通用「刪 node → 清邊」路徑**(補 node-only 的 dangling-edge 陷阱,`graph_sync` 目前只對 `_ENTITY_TYPES` 觸發清邊)+ 通用 `NodeOut` 序列化。做完用戶即可自訂層(純 node),task/project 不動。
  ...
  - [x] **A5 — 註冊表驅動角色判定:** `node_types` 加 `is_container`/`is_task_like` 兩旗標(migration `e5f7a9b1c3d4`,seed:project=container、task=task_like);六支 leaf helper(`member_project_ids`/`contained_task_ids`/`parent_task_map`/`project_ids_map`/`subtask_ids_among`/`top_level_task_filter`)改讀角色 —— Python-object 過濾用 `container_type_keys`/`task_type_keys`,SQL 用 join `node_types`。**內建角色集合與現行完全一致,故行為逐位元不變。** `NodeTypeOut` 唯讀曝露兩旗標;**`is_container` 本切片不開放使用者設定**(自訂 container 的 id 會混進 compat `project_ids` 被前端當 project 解析而 404,留待 Phase C 把 `project_id` 泛化為「父容器」時處理);`is_task_like` 對 node-only 型安全(仍走 Task row 物化)但也暫不從 create/update API 開放,保持純資料化重構。
  - ✅ **A6 — 前端(完成):**
    - [x] **A6.1 型別管理頁** `/graph-types`(`pages/GraphTypes.jsx`)—— list 內建詞彙 + create/delete 自訂 node/edge 型;內建鎖定;角色徽章(container/task/containment/symmetric)。vitest 6。commit `b7b18d4`。
    - [x] **A6.2 通用 node 檢視** `/explorer`(`pages/NodeExplorer.jsx`)—— 選型別 → 列出/建立/刪除自訂型 node;選 node → 檢視其邊(進/出)並 attach(rel_type + 目標型別/節點選單)/ detach,實現跨層自由包含;內建實體型唯讀。vitest 5。commit `909309c`。
    - [x] **A6.3 「未分類」bucket + 放寬守衛** `/unfiled`(`pages/Unfiled.jsx`)—— task 可合法零歸屬:後端放寬 `remove_membership`「最後一個歸屬」守衛(移除最後一個 → 未分類)、新增 `graph.unfiled_task_ids` + top-level `tasks.task_ops_router`(`GET /tasks/unfiled`、`POST /tasks/{id}/memberships/{project}` 歸入);前端未分類清單 + 每列專案選單歸入。vite proxy 補 `/tasks`。後端 748/747 雙 DB、前端 217 綠。commit `536a6e3`。

> **🎉 ADR-0033 Phase A 全部完成(A1–A6)。** 地基(兩張註冊表 + 通用 node/edge API + 溯源 + 角色資料化)與前端(型別管理 / node 探索 / 未分類)皆已上線並雙 DB 綠。下一步是 Phase B(逐型把內建實體收成 node-only,label→cycle→goal→identity→task→project),屬大工程,另行規劃。
- **Phase B — 逐型收斂(一次一 type,兩套 DB 綠):** 內建實體逐一搬進 node(欄位→hot columns/`data`,endpoint 轉通用層,drop 表)。輕的先做(label/cycle/goal/identity),最後才碰 task/project(有 endpoint/MCP/前端/大量測試依賴)。
  - [x] **B1 — label(最輕,已完成):** label 收成純 `Node(type="label")` —— `title`=name,`color`/`type`/`description`/`decision_status`/`source` 進 `data`,專案歸屬改 `contains` 邊(project→label)。`services/graph.py` 新增 `LabelView` adapter(維持 `LabelOut` 相容)+ 完整 helper(`create_label`/`update_label`/`delete_label`/`get_label`/`labels_in_project`/`labels_for_task`/`labels_map`/`label_project_map`/`find_label_by_name`/`decisions`)。所有寫入點(`labels`/`external_api/labels`/`decisions`/`issue_sync`/`imports`/`assistant_tools`/`rules_engine`)+ 讀取點(`enrichment`/`share`/`agent_context`/`deps`)轉通用層;`db.query(Label)` 全數消除。`Label` ORM 類別 + `Project.labels` relationship + `graph_sync._ENTITY_TYPES` 條目 + `graph.ENTITY_BACKED_TYPES` 的 `label` 皆移除。migration `f6a8c0d2e4b5`:回填 node.data + project→label `contains` 邊(78 筆守恆)後 drop `labels` 表,upgrade/downgrade 於 dev DB 往返驗證。雙 DB 綠(SQLite 748 pass / 79.6% cov)。
  - [x] **B2 — cycle(已完成):** cycle 收成純 `Node(type="cycle")` —— `title`=name,`status`/`start_date` 為真欄位,`end_date`→node `due_date`,`description` 進 `data`,專案歸屬改 `contains` 邊(project→cycle)。`services/graph.py` 新增 `CycleView` adapter(維持 `CycleOut` 相容)+ helper(`create_cycle`/`update_cycle`/`delete_cycle`/`get_cycle`/`cycles_in_project`/`cycles_for_task`/`cycle_project_map`/`find_cycle_by_name`),並把 B1 的 `label_project_map` 抽成共用 `project_container_map`。寫入點(`cycles` 全套含 duplicate/compare)+ 讀取點(`analytics` burndown/velocity/cycle-burndown、`external_api/analytics` velocity、`issue_sync` 里程碑同步 `_task_primary_cycle`/`apply_inbound_milestone_cycle`、`enrichment`、`share`、`deps`)轉通用層;`db.query(Cycle)` 全數消除。`Cycle` ORM 類別 + `Project.cycles` relationship + `graph_sync` 條目(含 `_sync_hot_fields` 分支)+ `graph.ENTITY_BACKED_TYPES` 的 `cycle` 皆移除。**額外修掉 B1/B2 共同的孤兒 bug:** `graph.delete_project_and_tasks` 現會刪除 project 所含的 label/cycle node(node-only 無 ORM cascade,否則刪 project 後 label/cycle node 會變孤兒);新增 `test_delete_project_deletes_its_labels_and_cycles` 回歸測試。migration `a7b9d1e3f5c6`:回填 node.data + project→cycle `contains` 邊(17 筆守恆)後 drop `cycles` 表(PostgreSQL 另 drop `cycle_status` enum),upgrade/downgrade 於 dev DB 往返驗證。雙 DB 綠(SQLite 749 pass / 79.6% cov)。
  - [x] **B3 — goal(已完成):** goal 收成純 `Node(type="goal")` —— `title`=title,`status` 為真欄位,`target_date`→node `due_date`,`description` 進 `data`。**與 label/cycle 不同:goal 非 project-scoped**(專案透過 `part_of` 邊掛到 goal,goal 本身無 `contains` 容器邊),故 `GoalView` 無 `project_id`、migration 也無容器邊要回填。`services/graph.py` 新增 `GoalView` adapter + helper(`create_goal`/`update_goal`/`delete_goal`/`get_goal`/`all_goals`,後者支援 status 過濾)。寫入/讀取點(`goals` 全套、`deps`)轉通用層;`db.query(Goal)` 全數消除。`Goal` ORM 類別 + `graph_sync` 條目(含 `_sync_hot_fields` 分支)+ `graph.ENTITY_BACKED_TYPES` 的 `goal` 皆移除。migration `b8c0d2e4f6a7`:回填 node 欄位/`data`(11 筆守恆,`part_of` 邊不動)後 drop `goals` 表(PostgreSQL 另 drop `goal_status` enum),upgrade/downgrade 於 dev DB 往返驗證。雙 DB 綠(SQLite 749 pass / 79.6% cov)。剩 identity → task → project。
  - [x] **B4 — identity(已完成):** identity 收成純 `Node(type="identity")` —— `title`=name,其餘全部(color/description/avatar/share_token/share_pin_hash/share_expires_at/allow_guest_notes)進 `data`(identity 熱欄位只有 title)。**與 goal 同為 top-level**(專案透過 `member_of` 邊、task 透過 `assigned_to` 邊掛上,identity 無 `contains` 容器邊)。棘手處:(1)`share_token` 是唯一查詢欄位,放進 JSON `data` 失去 DB unique/index,故用 `find_identity_by_share_token` 掃描 identity node 在 Python 比對(identity 量少);(2)`share_expires_at` 是 datetime,存 JSON 需 ISO 字串,`update_identity` 用 `_iso()` 寫入、`_identity_view` 用 `_parse_dt()` 讀回。`services/graph.py` 新增 `IdentityView` adapter + helper(`create_identity`/`update_identity`/`delete_identity`/`get_identity`/`all_identities`/`find_identity_by_share_token`),`identities_for_project` 改回傳 view。所有寫入點(`identities` 全套含 rotate-token/set-pin/clear-pin/set-expiry 皆改走 `update_identity`)+ 讀取點(`share` by-token/build/log-view、`bulk` ical-by-token、`external_api/summary`、`enrichment`、`deps`)轉通用層;`db.query(Identity)` 全數消除。`Identity` ORM 類別 + `graph_sync` 條目 + `graph.ENTITY_BACKED_TYPES` 的 `identity` 皆移除(**現在只剩 project/task 是 entity-backed**)。測試:`conftest` 的 `sample_identity`/`pinned_identity` fixture 改用 graph helper(固定 share_token 靠 `update_identity`);測試中對 IdentityView 做 setattr 不會持久化的坑(`allow_guest_notes = True` → 改 `update_identity`)已修。migration `c9d1e3f5a7b8`:回填 node.data(9 筆守恆,member_of/assigned_to 邊不動)後 drop `identities` 表,upgrade/downgrade 於 dev DB 往返驗證。雙 DB 綠(SQLite 749 pass / 79.6% cov)。**剩最後兩個大的:task → project。**
  - [x] **B5 — task(最大的一塊,已完成,aggressive drop):** task 收成純 `Node(type="task")`。分三段各自綠 commit:**B5.1**(`0f4aa20`,migration `d1e3f5a7b9c0`)先把 node 補成完整鏡射(`graph_sync._task_data` 把所有非熱欄位寫進 `node.data`,並在 before_flush pin `callback_token`);**B5.2**(`b9e6410`)建 `graph.TaskView` adapter + `get_task`/`task_views_by_ids`/`task_views_for_ids`,證明 `enrich_task(view)==enrich_task(orm)` 不變式,但暫不切讀;**B5.3**(本段,原子刀)一次完成寫入集中化、讀取切換、FK 遷移、drop 表、移除 ORM。細節:
    - **hot columns**(title/status/priority/start_date/due_date/position/is_pinned/created_at/updated_at)為真 node 欄位;其餘 14 個(description/callback_token/webhook_secret/assignee/assigned_agent_key_id/reminder_sent_at[ISO]/time_estimate/time_spent/progress_pct/agent_notes/external_*)進 `data`。
    - **寫入集中化:** `graph.create_task` 改為直接建 `Node`(不再建 `Task` ORM,不靠 graph_sync 鏡射);新增 `graph.update_task(db, id, **fields)`(hot 欄位直寫、其餘進 data、`_apply_task_fields` 共用)。所有 mutation(`tasks`/`external_api/{tasks,progress}`/`bulk`/`webhooks`/`issue_sync`/`rules_engine`/`scheduler` reminder+SLA priority/`assistant_tools`)全改走 `update_task`;`deps.get_task_or_404` 回傳 `TaskView`。
    - **讀取切換:** 69 處 `db.query(Task)` 全消除。純熱欄位過濾/統計改 `db.query(Node).filter(Node.type=="task", ...)`;需非熱欄位的改 `graph.task_view` 包裝或 `task_views_*`。**JSON `data` 欄位的過濾用 Python 掃描避方言差**(`find_task_by_callback_token`、`find_task_by_external`、search 的 description ilike、analytics 的 time_estimate/time_spent、api_keys 的 assigned_agent_key_id、issue_sync 的 external_id/pr_url)。
    - **FTS:** `search_backend` 改索引 task node(SQLite FTS5 trigger 掛在 `nodes` 上、`json_extract(data,'$.description')`;PostgreSQL GIN 改 `nodes ... data->>'description' WHERE type='task'`)。
    - **FK 遷移:** 5 張表(comments/attachments/recurrence_rules/task_pull_requests/webhook_events)的 `task_id`/`template_task_id` FK `tasks.id`→`nodes.id`。migration `d3f5a7b9c1e2`:PostgreSQL DROP/ADD CONSTRAINT 重指 + `DROP TYPE task_status/task_priority`;SQLite 因未開 `PRAGMA foreign_keys` 直接 `drop_table("tasks")`(dangling FK 惰性)。upgrade/downgrade 於 dev DB 往返驗證(122 task node 守恆)。dev DB 已 `alembic upgrade head`。
    - **刪除語意:** 移除 `Task.comments`/`pull_requests` ORM cascade 後,`graph.delete_task_tree` 新增 `_delete_task_node` 顯式刪 5 張周邊表列再 `delete_node`(SQLite 不強制 cascade)。
    - **移除:** `Task` ORM 類別 + `graph_sync` 的 Task 分支(`_ENTITY_TYPES` 只剩 Project)+ `ENTITY_BACKED_TYPES` 的 `task`(**現在只剩 project**)。5 張表的 `task`/`template_task` relationship 移除。
    - **測試:** 115 處 `Task(...)` 建構 → `tests/factories.py::make_task`(建 node + contains 邊,回傳 decorate 過非熱欄位的 Node 以相容 `db.add`/`db.refresh`/欄位讀取);`find_task_by_external_id`/`find_task_by_title` 補 JSON 查詢;test_task_view/test_graph_sync/test_backup/test_nodes_api 依 node-only 現實改寫。雙 DB 綠(SQLite 757 pass / 80% cov)。剩最後一個:**project(B6)**。
  - [x] **B6 — project(最後一個,已完成):** project 收成純 `Node(type="project")` —— `title`=name、`status` 為真欄位,其餘(description/share_token/share_expires_at ISO/allow_guest_notes/agent_instructions/repo_url/wip_limits)進 `data`。`services/graph.py` 新增 `ProjectView` adapter(維持 `ProjectOut.model_validate` 相容)+ helper(`create_project`/`update_project`/`get_project`/`all_projects` 支援 status 過濾/`projects_by_ids`/`find_project_by_share_token` 掃描比對 token/`search_project` name+description);`projects_for_identity`/`projects_for_goal`/`project_of_task` 改回傳 view;`delete_project_and_tasks` 改走 `delete_node`。約 40 處 `db.query(Project)` 讀寫點(`projects`/`external_api/projects` 全套 CRUD、`deps`/`bulk`/`share`/`goals`/`identities`/`imports`/`issue_sync`/`scheduler`/`analytics`/`search`/`external_api/{stats,tasks,labels,summary,agent_context,analytics,search,helpers}`/`assistant_tools`)全轉通用層;`db.query(Project)` 全數消除。**`projects` 無任何 FK 指入**(`api_keys`/`assistant_conversations`/`activity_logs` 的 `project_id` 皆純字串),故 migration 無約束要重接。`Project` ORM 類別移除;`graph.ENTITY_BACKED_TYPES` 清空(`frozenset()`,`/nodes` 守衛退化為 no-op,徹底移除留待 Phase C);**`graph_sync` 整支退役**(最後一個 mirror 對象消失,`main.py` 的 side-effect import 一併移除)。migration `e2f4a6b8c0d3`:回填 node.data(17 筆守恆)後 drop `projects` 表(PostgreSQL 另 drop `project_status` enum),upgrade/downgrade 於 dev DB 往返驗證。測試:`tests/factories.py::make_project` 取代 `Project(...)`(建 project node,回傳 decorate 過非熱欄位 + `name` 的 Node);對 ProjectView setattr 不持久化的坑(`repo_url`/`allow_guest_notes`/`share_expires_at`)改走 `graph.update_project`/`graph.get_project`;test_graph_sync/test_backup/test_nodes_api 依 node-only 現實改寫。雙 DB 綠(SQLite 757 pass / 80% cov、PostgreSQL 綠)、前端 217 綠。**🎉 Phase B 全部完成 —— 每個一等實體皆為 node-only,`graph_sync` 消失、`ENTITY_BACKED_TYPES` 清空。**
- **Phase C — 清理:**
  - [x] **C1(已完成,commit `cd70d71`):** `_ENTITY_TYPES` 已隨 `graph_sync` 退役消失;移除 `/nodes` 的 `_reject_entity_backed` 守衛 + 空的 `graph.ENTITY_BACKED_TYPES` 常數 —— 內建型現為一般 node type,generic node API 一視同仁(`test_nodes_api` 改測 project 可經 `/nodes` 建立)。
  - [x] **C2(已完成,commit `cd70d71`):** `enrichment.enrich_task`/`enrich_task_as_dict`/`enrich_project` + helper 的 `db` 改為必要參數,移除所有過渡期 `db is None` fallback 分支(真實呼叫端都已傳 session)。
  - [x] **C3 —— 視為已被 A5 滿足:** 角色過濾(container/task_like)已於 A5 資料化;其餘約 51 處字面 `NODE_TASK`/`NODE_PROJECT` 讀取是明確的直接型別查詢,不值得再加註冊表間接層。
  - [x] **C4a(已完成,commit `1db69d6`,ADR-0034):** 採「compat `project_id`/`project_ids` 只認字面 `project` 型 + 另加通用 `container_ids`」路線(非泛化 project_ids)。`graph.member_project_ids`/`project_ids_map` 改以 `Node.type == NODE_PROJECT` 過濾(不再跟 `is_container` 角色),自訂 container 的 id 永不外洩進 compat 專案欄位、前端不會 404;新增 `member_container_ids`/`container_ids_map` helper + `TaskOut.container_ids`(is_container 超集,含專案與自訂容器);刪除孤兒邏輯(`delete_task_tree`/`delete_project_and_tasks`)改用通用容器集(任何容器都能留住 task)。今日行為逐位元不變(內建唯一容器就是 project)。雙 DB 綠(SQLite 763 / PostgreSQL 762)。
  - [x] **C4b(已完成,ADR-0034):** 開放 `is_container` 給使用者設定 —— `NodeTypeCreate`/`NodeTypeUpdate` 納入 `is_container`;`POST/PATCH /graph-types/nodes` 可建立/切換自訂容器型,**內建型角色旗標不可變更**(改 built-in project 的 is_container 回 400);前端型別管理頁(`/graph-types`)node 建立表單新增「容器」核取方塊。有 C4a 保護,自訂容器安全落地。
  - [x] **C4c(已完成,ADR-0035)—— `is_task_like` 使用者可設定,自訂型為一等 task:** 產品決策 = 自訂 task-like 型別是**完整的一等 task**(出現在清單/看板、走同一條 enrichment、帶自己的型別 badge)。task 生命週期改角色驅動(`get_task`/`task_views_by_ids`/`update_task`/`_delete_task_node`/`find_task_by_*`/`set_parent_task` + router `list_tasks`/`reorder_tasks` 讀 `task_type_keys(db)`,僅 `create_task` 保留字面 NODE_TASK);`create_node` 對 task-like 型補完整 task `data`(callback_token + scalar,共用 `_apply_task_data_defaults`);`TaskOut` 加 `type`、`callback_token` 放寬為 `str|None`;`NodeTypeCreate/Update` 納入 `is_task_like`,PATCH 對內建型角色變更回 400;前端 GraphTypes task 核取方塊 + `TaskIcons.TypeBadge`(取自 `['node-types']` 快取,查無退回 key)。**設計說明:** 用資料欄位角色而非 class 繼承,因自訂型只存在於執行期、class 需編譯期;角色是可組合正交布林(mixin 而非單一繼承),行為仍封裝於 `TaskView` 由角色選取(組合優於繼承)。雙 DB 綠 + 前端 223 綠。
...
- [x] **提案 6 StructureMap 完整泛化(2026-07-18 完成):** 解法 = **「新推導、同輸出形狀」** —— `utils/graphStructure.js` 的 `deriveGraphStructure(slice, nodeTypes, edgeTypes)` 從 `GET /graph/map?include=data`(node.data 全量)+ 型別註冊表推導出與舊 `deriveStructureMap` 完全同形的輸出,enrich 欄位(progress/risk/decision 計數/share 旗標)全部改在 client 由圖計算,視覺層(territory/mindmap/network)幾乎不動。角色資料驅動:**任何 `is_container` 型都是一張卡**(map-role `type` 保持 `'project'`,真實型別走 `typeKey/typeLabel/typeColor/isCustomType`)、任何 `is_task_like` 型都是任務點、自訂 plain node 上 network 外環、自訂 edge 型畫成一般關聯線、容器巢狀(`parentContainerId`)畫 contains 線。`focusGraph` 取代 identity focus 的 `filterProjects`。routing 容器感知(自訂容器 → `/c/`)。後端補 `include=data` 參數與邊排序(position, created_at,「第一個容器」決定性)。`graphStructure.test.js` 9 項純函式測試。舊 `deriveStructureMap` 保留但頁面已不再使用。
