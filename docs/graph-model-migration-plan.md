...
- **Phase A — 地基(先交付用戶自訂層):** `node_types` 註冊表(type 資料化、seed 內建)+ 通用 Node CRUD API + **通用「刪 node → 清邊」路徑**(補 node-only 的 dangling-edge 陷阱,`graph_sync` 目前只對 `_ENTITY_TYPES` 觸發清邊)+ 通用 `NodeOut` 序列化。做完用戶即可自訂層(純 node),task/project 不動。
  ...
  - 🟡 **A6 — 前端(進行中):**
    - [x] **A6.1 型別管理頁** `/graph-types`(`pages/GraphTypes.jsx`)—— list 內建詞彙 + create/delete 自訂 node/edge 型;內建鎖定;角色徽章(container/task/containment/symmetric)。API client helper、route、sidebar(System 群)、en/zh-TW i18n、vitest 6 項。前端 207 綠、eslint 0 error、vite build 成功。commit `b7b18d4`。(vite proxy 已於 A2/A3 補;dev frontend 已重啟套用。)
    - [ ] **A6.2 通用 node 檢視** —— 列出/建立/檢視自訂型 node 與其 edge(打 `/nodes` API);可掛 contains 邊做自由包含。
    - [ ] **A6.3 「未分類」bucket + 放寬守衛** —— 未分類任務清單(零 project 歸屬)+ 從那裡指派;放寬 `remove_membership` 的「最後一個歸屬」守衛使 task 可合法歸零(接「未分類任務」線)。
