...
- **Phase C — 清理:** 移除 `_ENTITY_TYPES`(已隨 `graph_sync` 退役消失);移除 `/nodes` 的 `ENTITY_BACKED_TYPES` no-op 守衛;hardcode type 過濾改由註冊表驅動;實體 ORM 類別消失。compat `project_id` 屆時轉為通用「父容器」概念(呼應「未分類任務」)。
