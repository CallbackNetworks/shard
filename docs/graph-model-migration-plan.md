...
- **Phase C — 清理:**
  ...
  - [ ] **C4 —— 使用者決定暫緩(2026-07-18,產品面 + 高風險,遷移視為實質完成):** (a) 讓 `is_container`/`is_task_like` 可經 NodeType create/update API 設定(schema 已有欄位但目前只 seed/DB);(b) 把 compat `TaskOut.project_id`/`project_ids` 泛化為「父容器」——⚠️目前前端把 project_id/project_ids 當「真的 project」用(~10 個元件),自訂 container 的 id 一旦流入 `project_ids_map`→`project_ids` 會讓前端 `/projects/{id}` 404(正是 A5 不開放 `is_container` 的原因)。修法:前端支援通用容器(全套、風險高)或 compat `project_id` 只認 `project` 型祖先、另加獨立 `container_ids` 欄位。需獨立、帶測試的 session,嚴禁 big-bang。
