...
- **Phase C — 清理:**
  ...
  - [ ] **C4c(延後)—— `is_task_like` 使用者可設定:** 本次刻意不開放。`is_task_like` 會讓自訂型 node 被 `contained_task_ids`→task enrichment 管線當成 task 拉進專案清單、物化成半空 `TaskView`,且需要一整套「自訂 task-like 型如何在 task UI 呈現」的產品決策 —— 屬另一條更高風險的軸,待獨立、附產品決策的一次處理(見 ADR-0034)。
