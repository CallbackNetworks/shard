...
**A shareable node has one share panel** (ADR-0070): every share write for every shareable type — token, PIN, expiry, guest notes — is a `/api/nodes/{id}/share/*` endpoint, and the view count has one implementation in `services.activity.share_view_count` that `/identities/{id}/share-views` and `/projects/{id}/share-views` both delegate to. It matches `share.viewed` rows under any of `identity_id` / `project_id` / `node_id`, because a node can be reached through more than one facade; the three facades log through one `_maybe_log_share_view` in `routers/share.py`. On the frontend the single panel is `components/NodeShareFacet.jsx`, which reads share state from either shape it is handed (a raw `Node` keeps it under `data`, an enriched `IdentityOut`/`ProjectOut` flattens it) rather than making callers shim. `ProjectDetail` still has its own guest-notes toggle — the last unconverted copy.
