...
**A type declares which of its `data` keys are the user's** (ADR-0074): `node_types.fields` is a list of `{key, label, kind}` specs, seeded for built-ins in `services/graph_registry.py` beside `roles` and backfilled by the migration (the seed only inserts *missing* types, so it would skip every existing database). `data` otherwise mixes three unrelated things — user fields, feature machinery (`share_token`, `callback_token`, `reminder_sent_at`, …) and ad-hoc keys somebody wrote once — which is why no generic editor existed and each built-in grew its own page. `MANAGED_DATA_KEYS` in the same module lists what may never be declared editable; declaring one, or an unknown `kind`, is a 422. The declaration is descriptive so far: the generic editor is not built yet, and `PATCH /api/nodes/{id}` is *not* yet restricted by it — enforcement lands with the editor.
