...
**The repo is mirrored to `github.com/CallbackNetworks/shard`** — a *push* mirror, so every push from Gitea overwrites GitHub's branches and a commit made on GitHub (a merge button included) is erased by the next sync. Pull requests are applied upstream, never merged there; `README.md` and `CONTRIBUTING.md` say so, because the silent-erase is the failure mode. GitHub runs the same workflow on its own runners with no variables set — verified on the first sync: seven check jobs green, publish/deploy skipped.
...
**The self-hoster's data is in a volume, and the dev stack's is not** (ADR-0138 → ADR-0139): a bind-mounted host directory carries the *host's* ownership, and a path Compose has to create comes out `root:root` while the backend runs as uid 1000 — which is why the documented one-command install died with `unable to open database file` on any clone that had never run the dev stack, from ADR-0117 until it was finally run by somebody standing where a stranger stands. The self-host stack now uses named volumes: Docker seeds one from the image's own `/app/data`, which `Dockerfile.prod` creates and chowns, so it arrives owned by the process that writes it on every host and `SHARD_UID` is gone. The cost is that "copy those two directories" stopped being true — Settings → Backup and a documented `docker run … tar` replace it, and `down -v` is now the destructive command. `data/` stays tracked via `.gitkeep` for the **dev** stack, which still bind-mounts it. The `selfhost-install` CI job performs the documented install on every push — copying the tree to an absolute *host* path through a helper container, because this runner is docker-out-of-docker and the daemon, not the job, resolves the compose file's paths — and asserts the database is owned by the app user and survives a `down`. Only `publish-public` depends on it: a broken install must not reach strangers, and must not block production's deploy (ADR-0137).

**A self-hoster can pull instead of build** (ADR-0137): the `publish` job has a second half gated on `vars.PUBLIC_REGISTRY_URL`, publishing the images the integration job already proved — re-tagged, never rebuilt. A version tag is immutable (`docker manifest inspect` decides; bumping `pyproject.toml` is what cuts a release) and `latest` follows main. Public names are flat — `<ns>/shard-backend` — because Docker Hub repositories have no nested path; the private registry keeps its own three-level prefix and the two schemes do not have to agree.

**An upgrade stops first and keeps what it replaces** (ADR-0140): `scripts/upgrade.sh` is build → **stop** → **snapshot** → migrate → start. Stopping is what makes the snapshot worth taking — a WAL-mode SQLite file copied from under a running process is torn, and a torn file is not a rollback — and it means the migration runs with nothing else writing. The snapshot goes to the `shard-snapshots` volume via `--volumes-from`, so it cannot name the wrong volume, and the printed restore command looks the container up rather than hardcoding an id `up -d` is about to replace. It is a rollback, not a backup: five upgrades' worth, living beside the thing it protects. `scripts/diagnose.sh` is the other half — what a bug report needs, with setting *names* and never their values.
...
**The decision graph is a view you can act on** (`utils/decisionGraph.js` + `components/decisions/DecisionGraph.jsx`, ADR-0128): a graph view existed at `/structure` (NETWORK layout, plus its own `MAP`/`DEPENDENCIES` mode), but `DEPENDENCIES` draws `depends_on` — declared `task -> task` — and the decision lane filtered out anything with `status !== 'proposed'` as "noise", which meant production's two `supersedes` edges, whose endpoints are *both* settled, could never appear: the one piece of decision history in the database was structurally invisible. Three changes, together, because none of them alone puts an edge on screen. **(1)** `GoverningDecisions` is writable (`editable` on `/n/{id}` and in `MembershipPanel`) — `governs` had one control in the whole app, on the decision side, while the act is almost always noticed from the work ("why am I doing this?"); read-only it still renders nothing when empty, but writable the empty state *is* the point, since hiding it hides the only thing that would create something to show. **(2)** The decisions page gets a `list`/`graph` mode. Two bands, deterministic, **not a force layout** — the four questions are directional and position is the answer: column 0 is the foundations, following an arrow rightwards follows a premise to its conclusion, governed work sits below. Unconnected records are excluded and **counted** ("99 with no relations — show"), never silently dropped. Selecting a node dims what it does not touch and draws the same `DecisionCard` the list draws, so seeing a missing edge and adding it is one screen. Relations are told apart by stroke pattern and glyph, never hue (ADR-0088: this page spends colour on status); only `conflicts_with` gets weight, and it draws **no arrowhead** because the claim is symmetric even though the row is stored one way. **(3)** The structure map keeps a settled decision that an edge on screen touches — a node an edge points at is the definition of not-noise — and the `.slice()` cap keeps endpoints first, since dropping one leaves a line ending in nothing. Same pass: `.kt-map-segment` now carries its own padding and `is-active` style instead of inheriting them from `.kt-map-toolbar button`, which only surfaced the first time it was mounted somewhere else.

**A decision names its premises and its conflicts** (`graph_registry` + `services/graph/decision_records.py`, ADR-0127): ADR-0118 gave a decision `supersedes` and `governs`, and production then ran with **103 records, 2 `supersedes` edges and 1 `governs`** — 98 decisions pointing at nothing and pointed at by nothing. Not because nobody linked them, but because the two relations a decision most often actually has had nowhere to live: `supersedes` *retires* the far end, and `depends_on` is declared `task -> task` with the meaning "blocked until done", which a decision never is. Two new built-ins, both `decision -> decision`: **`requires`** (this holds only while that does; the reverse read `required_by` answers "what falls over if this premise is reopened") and **`conflicts_with`**. The second is where `edge_types.is_symmetric` finally *does* something — it has existed since ADR-0033, been served on both API doors and frozen on built-ins while no code read it, i.e. a comment shaped like a column. `graph.add_edge`/`remove_edge` read it now: the reverse row **is** this edge, so writing from the other side returns the existing row and deleting from either end works. `links_map` reads both directions into one list, because a record answering "no conflicts" while the record it contradicts says otherwise leaves one of two ends telling the truth and nothing saying which. Writes go through the generic edge surface — ADR-0118's exception is `supersedes` alone, and it earns it because the edge and the far end's status are one act. Creation sits behind the card's `⋯` (ADR-0122 weighted that row by how often the act happens; four labelled relation buttons is the wall it removed), and all three decision→decision relations share **one** `DecisionLinkPicker` — three modals differing only in a heading is three places to fix when the list needs a search box, which at a hundred records it did. `supersedes` stays inside one project; the other two reach across on purpose, since an organization-level premise for a project-level decision is the interesting case. A conflict is a question, not a verdict: resolve it by superseding one side, never by deleting either, so there is no "resolve" button. Note for any code reading `edges` directly (`/graph/map`): symmetric is a read-and-write convention, not a storage one — the row is still directed, so a renderer must know not to draw an arrowhead on `conflicts_with`.
...
**A decision lands where it is read** (`services/graph/decision_records.py`, ADR-0130): ADR-0118 moved a decision out of `label` + `data.type="decision"` and asserted the old shape would stop working *visibly*. It returned **201** and made a label — invisible to `decisions()` (which filters `Node.type`) and, since the same ADR removed `label_names`'s subtraction, a real entry in the label vocabulary. Production collected **17 in two days**: the newest 17 decisions in the database, none on the decisions page, 17 of its 22 labels. `assert_decision_write_shape` now refuses both misfiled shapes at `node_admin.create` and `dispatch_node_updated` — one call each covers both doors — and the refusal *names* `type='decision'`, because an agent always reads the error and not always the docs (ADR-0078). Same pass, the state moved to the **`nodes.status` column** like every other type's: it rode in `data["decision_status"]` from the label era, leaving the column NULL on all 103 records and making decision the one type a generic node filter could not narrow by state. The response contract is unchanged — reads still say `decision_status`, and `api/client.js`'s `decisionBody()` does that translation once — but sending it as a data key is a 422 too, since accepted silently it writes an inert key while the column every surface reads stays empty. Migration `d8f0a2c4e6b1` converts the 17, backfills the column, and turns two stale `data["superseded_by"]` keys into the real `supersedes` edge *before* dropping them.

**What dies with a node** (`services/node_teardown.py`, ADR-0131): `delete_node` dropped the node and its edges and nothing else, while `_delete_task_node` carried a hand-written list of five peripheral tables — the only place that knew side rows existed. So deleting one project stranded eight kinds of row at once (guest notes, saved filters, templates, notifications *including ones pointing at tasks the same delete removed*, integrations, share chat log, workflow rules), none of it loudly: a notification survives as a bell entry linking to a 404, a template survives in the global list, which is unscoped by default. One list, applied inside `delete_node`, so every type gets it. The split is **belonging, not mentioning**: `activity_logs` and `graph_events` name nodes and stay (ADR-0073 — retiring a subject must not retire its history), and `api_keys.container_id` stays because a credential is not a side row and a container-scoped key whose container is gone already fails closed. Order matters twice: attachment *files* before their rows, and a delivery log with its integration (ADR-0085 made those headers a second path out for a credential). `TestNothingIsLeftBehindByADelete` enumerates the whole table list rather than spot-checking, because the defect was never a forgotten table — it was that no list existed. Same pass dropped `assistant_messages.tool_call_id`: never written, never read, never served, for a `tool` message role this assistant does not create.

**A field declaration can be declared** (`components/graph/NodeTypeFieldsEditor.jsx`, ADR-0132): `fields` is what the generic node editor draws (ADR-0074) and is writable through both API doors, while `/graph-types` — the one screen about the registry — could edit key/label/colour/roles and not this. So a custom type could never gain an editable field: production's own `repository` layer carries **8 real fields across 9 nodes with zero declarations**, every one read-only on `/n/{id}`. Same class as ADR-0122's `governs`, which had a write helper, a reverse read and no caller. Built-in declarations stay frozen (ADR-0121) and render as a read-only summary. The vocabulary is **served, not mirrored**: `GET /api/graph-types/fields/vocabulary` returns `{managed, kinds, stores, columns}` from the code that enforces it, replacing `/data-keys/managed` (two endpoints serving halves of one vocabulary is the next drift). `columns` matters most — a `column` field naming anything outside `WRITABLE_COLUMNS` is written into `data` under the same name and looks saved while the column never changes — so picking `column` turns the key box into a picker rather than leaving a free box that can be wrong. The same pass closed a drift the new test found: `NodeFieldSpec` refused `MANAGED_DATA_KEYS` but not `node_data.DERIVED`, so `share_pin_set` could be declared into a widget whose every save is discarded.

**A picker is built from the rule the write path enforces** (`services/graph_registry.relation_options`, ADR-0150): ADR-0078's endpoint declarations were enforced on every `add_edge`, served on both doors and drawn on `/graph-types` — and the three screens that actually *create* an edge each re-derived the answer and got the same three things wrong. Every relation was offered on every node (on a project, **two of nine** could succeed); the edge always pointed outward, so `owns` and `governs` — which have the container at their *target* end — were unreachable from a project at all, while "this task belongs to that project" could only be said as `task contains project`, which `contains` **accepts**, storing containment backwards with no error; and candidates were every node in the database. `GET /graph-types/edges/options/{type}` (+ its v1 twin, + `list_edge_types(for_type=…)`) runs `graph.relation_accepts` — the predicate `add_edge` itself calls — once per direction, and resolves the far end to concrete type keys so no client needs a copy of the role table (ADR-0056). A symmetric relation yields one option, since the reverse row *is* the edge. `components/shared/RelationPicker.jsx` is the one control, replacing all three forms: direction lives *inside* the option list (two `optgroup`s, because "contained by" cannot be conjugated from a user-supplied label across languages), and the relation's `description` — served all along, drawn only where you could not act on it — appears at the moment of choosing. `tests/test_relation_options.py` pins both directions of the agreement: every offered option must be an edge `add_edge` takes, **and** every edge it takes must be offered — that second half is the gap that hid `owns`. Note `contains` itself is unchanged: `task contains project` is still accepted, it is simply no longer the only way to express the opposite.

**Data is one page, not four** (`pages/NodeExplorer.jsx`, ADR-0150): the DATA rail group spent four rows on four views of one dataset. The explorer had **no search box** (`getNodes` took a `query` the page never passed), took the endpoint's default page of 100 and printed *that* as the count — 144 tasks read as "100 nodes", 44 of them unreachable — and defaulted to `nodeTypes[0]`. `/unfiled` asked "no incoming containment edge", which is also true of every root, so an organization holding 21 projects sat in an inbox permanently under a hint telling you to file it somewhere. `/containers` was a two-card menu of container *types*, excluding `project` and `identity`, i.e. a strictly weaker copy of the left column of `/graph-types`. Now: one search across types, per-type totals from `usage_count` (a server COUNT, so it cannot move with the page size), real `offset` paging, and the selected node's relations editable in place. `graph.unfiled_node_ids` redefines loose as **nothing above it and nothing below it** — a root holds work and is not loose — and it is a filter here (`?loose=1`), with `?type=` beside it in the URL (ADR-0083). Both retired paths redirect; a retired page must not become a 404 and must not stay a second implementation. `EdgeEndpointsEditor` closes the last half: a custom relation's `allowed_source`/`allowed_target`/`is_symmetric`/`description` were writable by the API and by nothing clickable (the ADR-0132 shape), so every relation made from the UI was permanently unconstrained — which now also means permanently unhelpful to the picker. Built-ins stay frozen (ADR-0121), so they get no pencil.
