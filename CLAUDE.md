...
**A bind mount the clone does not carry is created by root** (ADR-0138): `data/` and `uploads/` are tracked (via `.gitkeep`, contents still ignored) because a path Compose creates itself comes out `root:root`, and the backend runs as uid 1000 — so the documented one-command install died on any clone that had never run the dev stack, with `unable to open database file`, since ADR-0117 shipped. `user: "${SHARD_UID:-1000}:${SHARD_GID:-1000}"` on the self-host backend covers a host whose first account is not 1000. The `selfhost-install` CI job now performs the documented install on every push — it copies the tree to an absolute *host* path through a helper container, because this runner is docker-out-of-docker and a relative bind mount is resolved by the daemon, not by the job. Only `publish-public` depends on it: a broken install must not reach strangers, and must not block production's deploy (ADR-0137).
