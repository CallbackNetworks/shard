...
**A shareable node has one share panel, and one public door** (ADR-0070, ADR-0071): every share write for every shareable type — token, PIN, expiry, guest notes — is a `/api/nodes/{id}/share/*` endpoint, and the view count has one implementation in `services.activity.share_view_count` that `/identities/{id}/share-views` and `/projects/{id}/share-views` both delegate to. Publicly, an identity (and any custom shareable container) is served only by `GET /share/node/{token}` + `/verify` + `/notes`, all of which dispatch on the node's type; the identity-flavoured paths and `/ical/identity/` are retired, and `/ical/node/` covers them. **The page and the call it makes are different paths on purpose**: the page is `/share/n/{token}`, the data is `/share/node/{token}` — same URL for both and the SPA answers its own fetch with `index.html` (ADR-0071). Any new root-level client call must be added to `frontend/backendPaths.js` *and* `nginx.conf`; `backendPathClaims.test.js` checks both directions. The view count matches `share.viewed` rows under any of `identity_id` / `project_id` / `node_id`, because a node can be reached through more than one facade; the facades log through one `_maybe_log_share_view` in `routers/share.py`. On the frontend the single panel is `components/NodeShareFacet.jsx`, which reads share state from either shape it is handed (a raw `Node` keeps it under `data`, an enriched `IdentityOut`/`ProjectOut` flattens it) rather than making callers shim. A PIN is enforced on **every** shareable type including projects (ADR-0072) — `ProjectView.share_pin_hash` exists so the page can see it, and the note gate uses the same hash so it cannot be a way around the page gate. `ProjectDetail` still has its own guest-notes toggle and share/iCal links — the last unconverted copy.
