...
**MCP Server** (`backend/app/mcp_server/server.py`): a module inside the backend package, not a separate build (ADR-0080) — the standalone container was a requirement of the stdio era, and remote HTTP (ADR-0076) retired it. Both entry points run the same module: `python -m app.mcp_server.server` for stdio (the client owns the process), `MCP_TRANSPORT=http` for the HTTP transport. It proxies all operations through `/api/v1` via httpx (see ADR-0005) — co-locating processes is a deployment decision, not a data-path one, and the API key's scope is what bounds the public endpoint. Provides 26 tools, 4 resources, 1 resource template, and 4 prompts — all declared with `MCPServer` decorators on `mcp` SDK 2.0, so a tool's **signature is its schema** (ADR-0077). There is no hand-written `inputSchema` and no `if name == ...` dispatch to keep in step; the thin decorated wrappers sit on top of the `_`-prefixed implementations, which are what the test suite mocks httpx against. A missing argument, an out-of-enum value or a failing tool is a protocol error, not a successful result whose text says "error".
...
**Configuring inbound CI/CD is not browser-only** (`services/webhook_credentials.py`, ADR-0084): the callback token *is* the address and the secret signs what arrives there, so revealing them is one deliberate, logged act (ADR-0059/0060) — and that act had a door only on the internal `/api`, which production puts behind `AUTH_PASSWORD`. It now has two: `/api/nodes/{id}/webhook` (+ `/rotate-secret`) and `/api/v1/nodes/{id}/webhook` (+ `/rotate-secret`, `admin` scope), both calling the same `reveal`/`rotate`. `admin` is not caution: the v1 redaction middleware strips `callback_token` for a lesser key, so a `write` key would get a config with the address silently missing. MCP exposes it as `manage_webhook`. The type rule, the lazy provisioning for containers (ADR-0082) and the activity row live in the service, so a third door cannot hand out a credential without recording it; each router keeps only its 404 and its own idea of who may ask.
