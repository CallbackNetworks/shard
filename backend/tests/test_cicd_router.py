...
def _make_task(db, project_id):
    t = Task(project_id=project_id, title="CI/CD test task", status="todo")
    db.add(t)
    db.commit()
    db.refresh(t)
    return t
...
def test_trigger_github(client, db, monkeypatch):
    ...
    r = client.post("/cicd/trigger/github", json={
        "repo": "owner/repo",
...
def test_trigger_gitlab(client, db, monkeypatch):
    ...
    r = client.post("/cicd/trigger/gitlab", json={
        "project_id": "12345",
        "ref": "main",
...
def test_trigger_jenkins(client, db, monkeypatch):
    ...
    r = client.post("/cicd/trigger/jenkins", json={
...
def test_trigger_generic(client, db, monkeypatch):
    ...
    r = client.post("/cicd/trigger/generic", json={
