...
def test_create_api_key(client):
    resp = client.post("/api-keys", json={"name": "My Key", "scopes": ["read", "write"]})
...
def test_create_api_key_missing_name(client):
    resp = client.post("/api-keys", json={"scopes": ["read"]})
    assert resp.status_code == 422


def test_create_multiple_keys(client):
    client.post("/api-keys", json={"name": "Key A", "scopes": ["read"]})
    client.post("/api-keys", json={"name": "Key B", "scopes": ["write"]})
    client.post("/api-keys", json={"name": "Key C", "scopes": ["read", "admin"]})
